...
{
    [AuthorizeAs("ModelRecord")]
    [ViewModel(SourceTables = new[] { "ModelRecord", "Role", "Permission" },
               Description = "Model-centric permission view for managing role access levels")]
    [UIEntity("Model Permission", ModuleName = "Governance", ShowInMenu = false, DefaultSort = ["RoleName|ASC"])]
    [PrimaryKey(nameof(RecordId), nameof(RoleId))]
    public class ModelPermission : TipsyBaboonModel, IModelWithSaveAction
    {
        ...
        [Column("RecordId", Order = 0)]
        [ForeignKey("ModelRecord")]
        public Guid RecordId { get; set; }

        [UIDisplay(Name = "Module", Order = 1, ShowInList = false)]
        public string ModuleName { get; set; } = string.Empty;

        [UIDisplay(Name = "Model", Order = 2, ShowInList = false)]
        [RecordName]
        ...
        [UIDisplay(Name = "Role Name", Order = 3, ShowInList = true)]
        public string RoleName { get; set; } = string.Empty;

        [UIDisplay(Name = "Description", Order = 4, ShowInList = false, ShowInDetail = true)]
        ...
        public async Task<SaveResponseType> BeforeChangeAsync(ActionType action, IServiceProvider serviceProvider, string? committingUser = null, CancellationToken cancellationToken = default)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"ModelPermission.BeforeChangeAsync error: {ex}");
                throw;
            }
        }
        ...
        public static ViewDefinition ViewDefinition => new()
        {
            ...
            Columns = new()
            {
                ...
                new() { SourceExpression = "ISNULL(p.CreatedOn, m.CreatedOn)", TargetProperty = "CreatedOn" },
                new() { SourceExpression = "ISNULL(p.CreatedByDisplayName, m.CreatedByDisplayName)", TargetProperty = "CreatedByDisplayName" },
                new() { SourceExpression = "ISNULL(p.CreatedById, m.CreatedById)", TargetProperty = "CreatedById" },
                new() { SourceExpression = "ISNULL(p.ModifiedOn, m.ModifiedOn)", TargetProperty = "ModifiedOn" },
                new() { SourceExpression = "ISNULL(p.ModifiedByDisplayName, m.ModifiedByDisplayName)", TargetProperty = "ModifiedByDisplayName" },
                new() { SourceExpression = "ISNULL(p.ModifiedById, m.ModifiedById)", TargetProperty = "ModifiedById" },
                new() { SourceExpression = "ISNULL(p.IsInvariant, 0)", TargetProperty = "IsInvariant" },
                new() { SourceExpression = "ISNULL(p.OwnershipLevel, m.OwnershipLevel)", TargetProperty = "OwnershipLevel" },
                new() { SourceExpression = "ISNULL(p.OwnerId, m.OwnerId)", TargetProperty = "OwnerId" },
                new() { SourceExpression = "ISNULL(p.GroupId, m.GroupId)", TargetProperty = "GroupId" },
